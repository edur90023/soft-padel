...
export const SettingsView: React.FC<SettingsViewProps> = ({ config, courts, users, onUpdateConfig, onUpdateCourts, onUpdateUsers }) => {
    ...
    const [playerForm, setPlayerForm] = useState<Partial<TournamentPlayer>>({ name: '', points: 0, matchesPlayed: 0, category: '' });
    ...
    // --- NUEVAS FUNCIONES DE GALERÍA Y RANKING ---
    ...
    const handleAddPlayer = () => {
        if (!playerForm.name || !playerForm.category) return alert("El nombre y la categoría son obligatorios.");
        ...
        setPlayerForm({ name: '', points: 0, matchesPlayed: 0, category: '' });
    };
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in">
                        ...
                        <div className="lg:col-span-2 space-y-4">
                            <div className="bg-slate-800/30 p-4 rounded-xl border border-white/5 flex justify-between items-center">
                                <span className="text-white font-bold">Intervalo de Rotación (seg)</span>
                                <input type="number" min="2" value={config.adRotationInterval || 5} onChange={(e) => onUpdateConfig({...config, adRotationInterval: parseInt(e.target.value)})} className="w-16 bg-slate-900 border border-white/10 rounded p-2 text-center text-white"/>
                            </div>
                    ...
                    <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in">
                        <div className="bg-slate-800/50 p-6 rounded-2xl border border-white/5 space-y-6">
                            <div className="flex items-center justify-between">
                                ...
                                <button onClick={() => onUpdateConfig({...config, promoActive: !config.promoActive})} className={`w-12 h-6 rounded-full transition-colors relative ${config.promoActive ? 'bg-green-500' : 'bg-slate-700'}`}>
                            ...
                            {config.promoActive && (
                                <div className="space-y-4 pt-4 border-t border-white/5">
                                    <div>
                                        ...
                                        <input type="number" value={config.promoPrice} onChange={e => onUpdateConfig({...config, promoPrice: parseFloat(e.target.value)})} className="w-full bg-slate-900 border border-white/10 rounded-lg p-3 text-white font-bold text-lg"/>
                                    </div>
                                    <div>
                                        <label className="text-xs text-slate-400 font-bold block mb-1">Texto Beneficio</label>
                                        <input type="text" value={config.promoText} onChange={e => onUpdateConfig({...config, promoText: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-lg p-3 text-white" placeholder="Ej: ¡Gaseosa Gratis!"/>
                                    </div>
                                </div>
                            )}
                        </div>
                ...
                {activeTab === 'gallery' && (
                    <div className="animate-in fade-in space-y-8">
                        <div className="flex justify-between items-center">
                            <h3 className="text-white font-bold text-lg uppercase italic tracking-tighter">Galería de Fotos</h3>
                            <label className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-2xl font-black uppercase text-xs cursor-pointer flex items-center gap-2 transition-all active:scale-95 shadow-xl shadow-blue-900/20">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                            {config.gallery?.map((img, i) => (
                                <div key={i} className="aspect-video rounded-2xl overflow-hidden border border-white/10 group relative bg-slate-900">
                                    <img src={img} className="w-full h-full object-cover"/>
                                    <button onClick={() => onUpdateConfig({ gallery: config.gallery.filter((_, idx) => idx !== i) })} className="absolute top-2 right-2 p-2 bg-red-600 text-white rounded-lg opacity-0 group-hover:opacity-100 transition-all hover:scale-105">
                                        <Trash size={16}/>
                                    </button>
                                </div>
                            ))}
                        ...
                        <div className="bg-slate-800/50 p-6 rounded-2xl border border-white/5">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
                                <div className="md:col-span-2">
                                    <input type="text" placeholder="Nombre Jugador" value={playerForm.name} onChange={e => setPlayerForm({...playerForm, name: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-white text-sm outline-none focus:ring-2 focus:ring-blue-500"/>
                                </div>
                                <div className="md:col-span-1">
                                    <input type="text" placeholder="Cat. (Ej: 7ma)" value={playerForm.category || ''} onChange={e => setPlayerForm({...playerForm, category: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-white text-sm outline-none focus:ring-2 focus:ring-blue-500 uppercase"/>
                                </div>
                                <div>
                                ...
                                <button onClick={handleAddPlayer} disabled={!playerForm.name || !playerForm.category} className="bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-black uppercase text-xs disabled:opacity-50 transition-colors">
                                    Agregar
                                </button>
                            </div>
                        </div>
                                            ...
                                            <tr key={p.id} className="hover:bg-white/5 transition-colors">
                                                <td className="p-4">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <span className="text-[10px] text-blue-400 bg-blue-500/10 border border-blue-500/20 px-2 py-0.5 rounded font-black uppercase tracking-widest">Cat. {p.category}</span>
                                                    </div>
                                                </td>
                                                <td className="p-4 text-center text-slate-400 font-mono">{p.matchesPlayed}</td>
